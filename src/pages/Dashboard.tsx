...
import { getUserData, calcularFuturo, UserData } from "@/lib/storage";
      ...
      <div className="px-5 py-6 pb-24 max-w-lg mx-auto">
        ...
        <div className="space-y-3 mb-6">
          {alerts.map((alert, i) => (
            <div
              key={i}
              className={`rounded-xl p-4 border flex items-start gap-3 animate-fade-up ${
                alert.type === "danger"
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            { label: "Renda", value: `R$ ${data.renda.toLocaleString("pt-BR")}`, icon: TrendingUp },
            { label: "Gastos", value: `R$ ${data.gastos.toLocaleString("pt-BR")}`, icon: TrendingUp },
            { label: "Saldo", value: `R$ ${saldo.toLocaleString("pt-BR")}`, icon: TrendingUp },
            { label: "Reserva", value: data.temReserva ? `R$ ${data.valorReserva.toLocaleString("pt-BR")}` : "Nenhuma", icon: Shield },
          ].map((item, i) => (
        ...
        <button
          ...
          className="w-full bg-card rounded-xl p-4 border shadow-sm flex items-center gap-3 hover:bg-muted/50 transition-colors"
        >
