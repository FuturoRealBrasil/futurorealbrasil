...
export function useCaixinhas() {
  ...
  async function createCaixinha(nome: string, imageFile?: File) {
    if (!user) return;
    ...
    await supabase.from("caixinhas").insert({
      user_id: user.id, nome, imagem_url,
    } as any);
    await load();
  }

  async function updateCaixinha(id: string, nome: string, imageFile?: File) {
    if (!user) return;
